...
const StockVehicleModule = {
    ...
    getHTML: function() {
        return `
            <style>
                ...
                .sv-sum-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-bottom: 20px; }
                .sv-sum-card { padding: 15px; border-radius: 12px; color: white !important; text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
                .sv-sum-card span { display: block; font-size: 10px; font-weight: bold; margin-bottom: 5px; text-transform: uppercase; }
                .sv-sum-card strong { font-size: 18px; display: block; }
                .sv-full-card { width: 100%; margin-top: 10px; padding: 20px; background: #1b5e20 !important; color: white !important; border-radius: 12px; text-align: center; font-size: 24px; font-weight: 800; }
            ...
            <div class="card no-print" style="padding-bottom:10px; margin-bottom:15px;">
                <div class="sv-nav">
                    ...
                    <button class="sv-tab-btn" id="btn-sv-ledger" onclick="StockVehicleModule.switchInternalTab('ledger')">📋 Ledger</button>
                </div>
            </div>
            ...
            <div id="sv-dash-page" class="sv-page card">
                ...
                <div class="no-print" style="display:flex; gap:10px; margin-bottom:20px; align-items:end;">
                    <div><label>From</label><input type="date" id="sv_dash_f_date" class="sv-date-input"></div>
                    <div><label>To</label><input type="date" id="sv_dash_t_date" class="sv-date-input"></div>
            ...
            <div id="sv-entry-page" class="sv-page card">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap:15px; align-items:end;">
                    ...
                    <div><label>Vehicle No</label><input type="text" id="sv_v_no" list="sv_v_list" onchange="StockVehicleModule.fetchOwner()" style="text-transform:uppercase;"></div>
                    <div><label>Owner Name</label><input type="text" id="sv_owner_name_auto" readonly style="background:#eee"></div>
                    <div><label>Wheel</label>
                        <select id="sv_wheel">
                            <option>6 Wheel</option><option selected>10 Wheel</option><option>12 Wheel</option><option>14 Wheel</option><option>16 Wheel</option>
                        </select>
                    </div>
                    <div><label>Trips</label><input type="number" id="sv_t_trips" value="0"></div>
                    <div><label>Diesel (L)</label><input type="number" id="sv_diesel" value="0"></div>
            ...
            <div id="sv-owner-page" class="sv-page card">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap:15px; margin-bottom:20px;">
                    <input type="text" id="sv_m_v_no" placeholder="Vehicle No (WB-XX-XXXX)">
                    <input type="text" id="sv_m_o_name" placeholder="Owner Name">
                    <button onclick="StockVehicleModule.saveOwner()" class="btn-action" style="background:#3498db;">REGISTER</button>
                </div>
                <table class="d-table" style="width:100%; border:1px solid #ddd;"><tbody id="sv-owner-table-body"></tbody></table>
            </div>
            
            <div id="sv-ledger-page" class="sv-page card">
                <div class="no-print" style="display:flex; gap:10px; margin-bottom:20px; align-items:end; flex-wrap:wrap;">
                    <div><label>Owner</label><select id="sv_o_selector" onchange="StockVehicleModule.loadLedger('owner')"></select></div>
                    <div><label>Vehicle</label><select id="sv_v_selector" onchange="StockVehicleModule.loadLedger('vehicle')"></select></div>
                ...
                <div id="sv-ledger-content" style="display:none;">
                    <div id="sv-ledger-header" style="text-align:center; border-bottom: 3.5px solid #1e3a5f; padding-bottom: 10px; margin-bottom: 20px;">
    ...
    renderOwnerTable: function() {
        ...
        document.getElementById('sv-owner-table-body').innerHTML = Object.keys(owners).map(v => 
            `<tr><td>${v}</td><td style="font-weight:bold;">${owners[v].name}</td><td class="no-print"><button onclick="StockVehicleModule.delOwner('${v}')" style="color:red; border:none; background:none; cursor:pointer;">Del</button></td></tr>`
        ).join('');
    },
    ...
    updateDashboard: function() {
        ...
        document.getElementById('sv-dash-container').innerHTML = `
            <div class="sv-sum-grid">
                ...
                <div class="sv-sum-card" style="background:#d35400"><span>Other</span><strong>₹${tO.toFixed(0)}</strong></div>
            </div>
            <div class="sv-full-card">Net Balance: ₹${((tT*1000) - (tD+tO)).toFixed(0)}</div>
        `;
    },
    ...
    loadLedger: function(type) {
        ...
        document.getElementById('sv_p_vno').innerText = selV || "Multiple Vehicles"; 
        ...
        document.getElementById('sv-ledger-data').innerHTML = entries.map(i => {
            ...
            return `<tr><td style="padding:8px;">${i.date.split('-').reverse().join('-')}</td><td style="padding:8px;">${i.wheel}</td><td style="padding:8px;">${i.trips}</td><td style="padding:8px;">${rate}</td><td style="padding:8px;">${i.diesel_ltr.toFixed(1)}</td><td style="padding:8px;">${dc.toFixed(0)}</td><td style="padding:8px;">${i.other_exp.toFixed(0)}</td><td style="padding:8px;" class="${b>=0?'bal-pos':'bal-neg'}">₹${b.toFixed(0)}</td><td class="no-print"><button onclick="StockVehicleModule.editEntry(${i.id})" style="color:blue; border:none; background:none;">Edit</button></td></tr>`;
        }).join('');
    },
