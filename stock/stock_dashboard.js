// ==========================================
// STOCK DASHBOARD & ARCHIVE MODULE
// ==========================================
var StockDashboardModule = {
    init: function() {
        const today = new Date();
        document.getElementById('sd_from').value = new Date(today.getFullYear(), 0, 1).toISOString().split('T')[0];
        document.getElementById('sd_to').value = today.toISOString().split('T')[0];
        this.generateReport();
    },
    ...
    getHTML: function() {
        return `
            ...
            <div class="card no-print" style="border-top-color: #8e44ad;">
                <h3 style="color:#8e44ad; margin-top:0;">📦 Stock Dashboard & Yearly Report</h3>
                <div class="form-grid" style="display:flex; gap:10px; align-items:end; background:#eef2f7; padding:15px; border-radius:8px; flex-wrap:wrap;">
                    <div style="flex:1"><label>From Date</label><input type="date" id="sd_from" class="sm-date-input"></div>
                    <div style="flex:1"><label>To Date</label><input type="date" id="sd_to" class="sm-date-input"></div>
                    <button onclick="StockDashboardModule.generateReport()" class="btn-action" style="background:#8e44ad;">Generate Report</button>
                    <button onclick="window.print()" class="btn-action" style="background:#e67e22; width:auto;">🖨️ Print Report</button>
                </div>
            </div>

            <div id="sd_report_area" style="display:none;">
                <div class="print-header">
                    <h1 id="sd_print_ghat_name">STOCK REPORT</h1>
                    <p id="sd_print_date_range"></p>
                </div>

                <div class="dash-stat-grid" id="sd_cards"></div>
                
                <div class="card no-print" style="margin-top:20px;">
                    <h3 style="margin-top:0; color:#333; border-bottom:2px solid #eee; padding-bottom:10px;">📅 Stock Daily Sheet Archive</h3>
                        ...
                        <table style="width:100%; border-collapse:collapse; border:1px solid #ddd;">
                            <thead style="background:#2c3e50; color:white;">
                                <tr>
                                    ...
                                    <th style="padding:10px;">Stock Expense</th>
                                    <th style="padding:10px;">Net Profit Joma</th>
                                    <th style="padding:10px;">Action</th>
                                </tr>
                            </thead>
                            <tbody id="sd_archive_body"></tbody>
                        </table>
                    </div>
                </div>
            </div>
    ...
    generateReport: function() {
        const from = document.getElementById('sd_from').value;
        const to = document.getElementById('sd_to').value;
        ...
        const sheets = (App.db.stock_ledger || []).filter(i => i.ghat === currentGhat && i.date >= from && i.date <= to);
        const exps = (App.db.stock_expenses || []).filter(i => i.ghat === currentGhat && i.date >= from && i.date <= to);
        const manuals = (App.db.stock_manual_adjustments || []).filter(i => i.ghat === currentGhat && i.date >= from && i.date <= to);

        // প্রিন্ট হেডারের ডেটা সেট করা
        document.getElementById('sd_print_ghat_name').innerText = currentGhat + " - STOCK REPORT";
        document.getElementById('sd_print_date_range').innerText = `Report Period: ${from.split('-').reverse().join('-')} TO ${to.split('-').reverse().join('-')}`;
        ...
        document.getElementById('sd_report_area').style.display = 'block';
        document.getElementById('sd_cards').innerHTML = `
            ...
            <div class="dash-card expense"><h4>Total Stock Expenses</h4><h2>₹${totalExp.toFixed(0)}</h2></div>
            ...
            archiveHtml += `
                <tr style="text-align:center; border-bottom:1px solid #eee;">
                    ...
                    <td style="padding:10px;" class="no-print">
                        <button onclick="StockDashboardModule.goToSheet('${d}')" style="background:#8e44ad; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer; font-weight:bold;">👁️ View Sheet</button>
                    </td>
                </tr>
            `;
        });
        
        if(archiveHtml === "") archiveHtml = `<tr><td colspan="6" style="padding:20px; text-align:center; color:#7f8c8d;">No Data Available</td></tr>`;
        document.getElementById('sd_archive_body').innerHTML = archiveHtml;
    },

    goToSheet: function(dateStr) {
        App.showPage('stock'); 
        setTimeout(() => {
            let dateInput = document.getElementById('stk-date');
            if(dateInput) {
                dateInput.value = dateStr;
                StockModule.updateDate(); 
            }
        }, 100);
    }
};